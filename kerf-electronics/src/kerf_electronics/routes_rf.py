...
async def run_openems_study(req: RFStudyRequest):
    """
    openEMS stub — not yet implemented.
    openEMS (http://openems.de) is a free FDTD electromagnetic field solver.
    When implemented, this function will:
    1. Write the touchstone data to a temporary .sNp file.
    2. Launch an openEMS simulation via python-openEMS bindings.
    3. Extract S-parameters from the simulation output.
    4. Return the same RFStudyResponse shape as run_rf_study.
    """
    return RFStudyResponse(
        status="error",
        errors=["openEMS integration is not yet implemented"],
        warnings=["scikit-rf backend is available; use /run-rf-study instead"],
    )
