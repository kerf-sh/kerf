...
- **Stability Factor K (Rollett K)** — device stability criterion; K > 1 implies unconditional stability
- **Max Available Gain (MAG, dB)** — maximum available gain computed from S-parameters (only valid when K > 1)
...
```json
{
  ...
  "result": {
    ...
    "stability_factor_k": [1.2, 1.5, 2.1, ...],
    "max_gain_db": [10.5, 12.3, 11.8, ...],
