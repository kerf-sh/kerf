...
| ↳ **T1: Binding probe + `surfaceToSolid` helper** | 📋 next | One-shot probe at `loadOcct()` boot logging presence of `BRepAlgoAPI_Common_3` / `BRepBuilderAPI_Sewing` / `BRepBuilderAPI_MakeSolid_1`. Adds `surfaceToSolid(oc, shape, tracker)` helper to `src/lib/occtBridge.js`. Gating: if `Sewing` is missing, escalate for OCCT WASM rebuild before T2+. If `MakeSolid_1` / `Common_3` missing, T2/T4 switch to fallback paths. Deps: none. |
