...
| **NURBS surfacing (Phase 4)** | ✅ shipped | sweep1/sweep2/networkSrf/blendSrf surface creation tools + display. Python NurbsSurface layer (`packages/kerf-cad-core/src/kerf_cad_core/geom/`). LLM tools `feature_sweep1/2/network_srf/blend_srf` + `surface_continuity` query/enforce tool (C0/C1/C2 for sweeps/network; G0/G1/G2 for blend). `.surf` file kind documented. NURBS tessellated for display by OCCT worker. **Scope intentionally bounded**: surface creation + display only. NURBS trimming/booleans deferred — see [docs/plans/nurbs-booleans-scoping.md](./docs/plans/nurbs-booleans-scoping.md): a ~2-3 week small-scope path (sweep-to-solid capping + `feature_boolean` op) exists but parked behind user-demand signal. Multi-year framing accurate for full NURBS-NURBS booleans + trim-by-curve + matchSrf + G3 continuity. |
