...
| ↳ Document the working sketch-import form in `llm_docs/jscad.md` | 📋 next | Replace the wrong `require('/x.sketch')` shape with the actual ES `import profile from '/x.sketch'` form the runtime implements. Add `.equations` → `params` injection section and a `.jscad` vs `.feature` comparison table. |
