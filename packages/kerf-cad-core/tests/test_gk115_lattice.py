...
Oracles (from spec):
1. Gyroid implicit is periodic: f(x + cell_size, y, z) ≈ f(x, y, z).
...
class TestGyroid:
    ...
    # Oracle: periodicity — f(x + L, y, z) ≈ f(x, y, z)
...
class TestOctetTruss:
    ...
    def test_no_duplicate_struts(self):
        ot = octet_truss(10.0, 0.3)
        # Each strut is a pair; ensure set of canonical pairs has len 36
        canonical = set()
    ...
    def test_node_count(self):
        ot = octet_truss(10.0, 0.3)
        # 8 corners + 6 face centres + 1 body centre = 15
        assert len(ot["nodes"]) == 15
