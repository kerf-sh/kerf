...
class NurbsCurve:
    degree: int
    control_points: np.ndarray   # shape (n, dim)
...
class NurbsSurface:
    degree_u: int
    degree_v: int
    control_points: np.ndarray   # shape (nu, nv, dim)
    knots_u: np.ndarray
    knots_v: np.ndarray
```
...
- The `curve_curve_intersection` finder is sample-based — increase `samples` for
  complex high-curvature curves.
